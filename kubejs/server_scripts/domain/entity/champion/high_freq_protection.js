 ...
 */
function ChampionHighFreqProtectionEntityBeHurt(customData, event, championKey, championLevel) {
    ...
    event.amount = Math.max(1, event.amount / Math.pow(2, counter))
...
RegistryChampionStrategy(
    new ChampionStrategyModel('high_freq_protection')
        .addStrategy('entity_be_hurt', ChampionHighFreqProtectionEntityBeHurt)
)
