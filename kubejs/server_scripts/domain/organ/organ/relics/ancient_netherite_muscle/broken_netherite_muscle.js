// priority: 500
RegistryOrgan('kubejs:broken_netherite_muscle')
    .addScore('kubejs:extreme_strength', 3)
    .addScore('chestcavity:endurance', -3)

/**
* @param {OrganEventCustomData} customData
* @param {Internal.LivingDeathEvent} event 
* @param {Internal.ItemStack} organItem
* @param {number} organIndex
* @param {string} slotType
*/
function BrokenNetheriteMuscleEntityKill(customData, event, organItem, organIndex, slotType) {
    const entity = event.entity
    if (entity.type != 'cataclysm:netherite_monstrosity') return
    if (entity.persistentData.getString('relicsStage') != 'relics') return
    SetChestCavityOrgan(customData, customData.killer.chestCavityInstance, Item.of('kubejs:netherite_muscle'), organIndex, slotType, true)
}

RegistryOrganStrategy(
    new OrganStrategyModel('kubejs:broken_netherite_muscle')
        .addOnlyStrategy('entity_kill', BrokenNetheriteMuscleEntityKill)
        .addOnlyStrategy('chest_cavity_update', RelicsOrganScoreChestCavityUpdate, 10)
)