 ...
 */
const GenChorusFruit = (level, sphere, offset, horizontalSpread) => {
    // todo 需要测试
    $ChorusFlowerBlock.generatePlant(level, sphere.center.offset(offset).above(), level.random, horizontalSpread)
}
...
const GlobalSetBiomeAction = (level, sphere, biomeName) => {
    ...
    for (let x = sphere.center.x - radius; x <= sphere.center.x + radius; x += 16) {
        for (let z = sphere.center.z - radius; z <= sphere.center.z + radius; z += 16) {
            // SetBiomeByPosChunk 内部会按 pos 重新计算 chunk 坐标，故传入代表该区块的任意位置即可
            SetBiomeByPosChunk(level, new BlockPos(x, 0, z), biomeName)
        }
    }
}