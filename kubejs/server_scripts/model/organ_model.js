...
publicField.NFWC.organManager.OrganList = []
const OrganList = publicField.NFWC.organManager.OrganList
publicField.NFWC.organManager.PseudoOrganList = []
const PseudoOrganList = publicField.NFWC.organManager.PseudoOrganList
...
publicField.NFWC.constructors.Organ = Organ
function RegistryOrgan(itemId) {
    let organ = new Organ(itemId)
    OrganList.push(organ)
    return organ
}
publicField.NFWC.organManager.utils.RegistryOrgan = RegistryOrgan
function RegistryPseudoOrgan(itemId) {
    let organ = new Organ(itemId).setPseudo(true)
    PseudoOrganList.push(organ)
    return organ
}
publicField.NFWC.organManager.utils.RegistryPseudoOrgan = RegistryPseudoOrgan
